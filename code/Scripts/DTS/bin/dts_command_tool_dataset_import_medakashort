...
my $rootSourceImages    = dir( "${DTS_DIR_PROJECTS}/${organism}/test_data_embryoImages/210519_Normal/A001" );
...
for my $source ( @rootSourceImagesGlobArray )
{
    ...
    my ($well_id, $loidstr, $embryo_id, $suffix)  = ($source =~ /\/img--(A\d+)--(LO\d+)--(E\d+)\.(.+)$/);
...
for my $source ( @rootSourceMatGlobArray )
{
    ...
    # ${DTS_DIR_PROJECTS}/MedakaShort/plots/210519_Normal--A001--E010/210519_Normal--A001--E010_sims_autoregression_gridZ.mat
    
    my ($some_id, $rank_id, $embryo_id)  = ($source =~ /\/([^\/]+)--(A\d+)--(E\d+)_sims_autoregression_gridZ.mat$/);

    warn '($some_id, $rank_id, $embryo_id)=', pp( [$some_id, $rank_id, $embryo_id] );
    ...
    my $target = $rootTargetMat->file( sprintf "TN_similarities_%s--%s--PO01--%s.mat", $some_id, $rank_id, $embryo_id );
